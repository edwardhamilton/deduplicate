...
class Test_Dedupe(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.filter = filter_TfidfCosSimilar_and_Distance.filter_TfidfCosSimilar_and_Distance(self.df, ngram_range=(1,3), min_name_vectorization_word_frequency=0.0,
    ...
    def test_with_training_full(self):
        result = dedupe.dedupe(data=self.df, partition=self.partition, match=self.match).run(num_processes=1, sample=100000)
